...
class fastMP3File {
    ...
    public function getDuration() {
        ...
        while (!feof($fd)) {
            $block = fread($fd, 10);
            if (strlen($block)<10) { break; } // EOF has been reached
            //looking for 1111 1111 111 (frame synchronization bits)
                ...
                // 4000 seems to be a nice balance between speed and accuracy
                if($i > 4000) {
                    break;
                }
    ...
    public static function parseFrameHeader($fourbytes) {
        ...
        $info = [];
        $info[] = self::framesize($layer, $bitrate, $sample_rate, $padding_bit);
        $info[] = $sample_rate;
